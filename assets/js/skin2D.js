...
function initScene() {
    try {
        ...
        setStatus('Erreur lors de l\'initialisation de la scène: ' + error.message, true);
...
function createTexture(originalTexture, x, y, width, height) {
    ...
    ctx.drawImage(
        ...
        x, y,         // Position source
        width, height, // Taille source
        0, 0,         // Position destination
        width, height  // Taille destination
    );
...
function applyTexture(texture) {
    try {
        ...
        if (isNewFormat) {
            ...
            const headOverlay = new THREE.Mesh(headOverlayGeometry, headOverlayMaterials);
            ...
            const bodyOverlay = new THREE.Mesh(bodyOverlayGeometry, bodyOverlayMaterials);
            bodyOverlay.position.y = 0;
            player.add(bodyOverlay);

            // --- BRAS GAUCHE OVERLAY ---
            // Coordonnées UV pour l'overlay du bras gauche
            const leftArmOverlayX = 40, leftArmOverlayY = 32;

            const leftArmOverlayMaterials = [
                createOverlayMaterialForFace(texture, leftArmOverlayX + 8, leftArmOverlayY + 4, 4, 12),  // Droite (+x)
                createOverlayMaterialForFace(texture, leftArmOverlayX + 0, leftArmOverlayY + 4, 4, 12),  // Gauche (-x)
                createOverlayMaterialForFace(texture, leftArmOverlayX + 4, leftArmOverlayY + 0, 4, 4),   // Haut (+y)
                createOverlayMaterialForFace(texture, leftArmOverlayX + 8, leftArmOverlayY + 0, 4, 4),   // Bas (-y)
                createOverlayMaterialForFace(texture, leftArmOverlayX + 4, leftArmOverlayY + 4, 4, 12),  // Devant (+z)
                createOverlayMaterialForFace(texture, leftArmOverlayX + 12, leftArmOverlayY + 4, 4, 12)  // Derrière (-z)
            ];

            // Créer l'overlay du bras gauche
            const leftArmOverlayGeometry = new THREE.BoxGeometry(4.5, 12.5, 4.5);
            const leftArmOverlay = new THREE.Mesh(leftArmOverlayGeometry, leftArmOverlayMaterials);
            leftArmOverlay.position.set(-6, 0, 0);
            player.add(leftArmOverlay);

            // --- BRAS DROIT OVERLAY ---
            // Coordonnées UV pour l'overlay du bras droit
            const rightArmOverlayX = 48, rightArmOverlayY = 48;

            const rightArmOverlayMaterials = [
                createOverlayMaterialForFace(texture, rightArmOverlayX + 8, rightArmOverlayY + 4, 4, 12),  // Droite (+x)
                createOverlayMaterialForFace(texture, rightArmOverlayX + 0, rightArmOverlayY + 4, 4, 12),  // Gauche (-x)
                createOverlayMaterialForFace(texture, rightArmOverlayX + 4, rightArmOverlayY + 0, 4, 4),   // Haut (+y)
                createOverlayMaterialForFace(texture, rightArmOverlayX + 8, rightArmOverlayY + 0, 4, 4),   // Bas (-y)
                createOverlayMaterialForFace(texture, rightArmOverlayX + 4, rightArmOverlayY + 4, 4, 12),  // Devant (+z)
                createOverlayMaterialForFace(texture, rightArmOverlayX + 12, rightArmOverlayY + 4, 4, 12)  // Derrière (-z)
            ];

            // Créer l'overlay du bras droit
            const rightArmOverlayGeometry = new THREE.BoxGeometry(4.5, 12.5, 4.5);
            const rightArmOverlay = new THREE.Mesh(rightArmOverlayGeometry, rightArmOverlayMaterials);
            ...
            const leftLegOverlayX = 0, leftLegOverlayY = 32;

            const leftLegOverlayMaterials = [
                createOverlayMaterialForFace(texture, leftLegOverlayX + 8, leftLegOverlayY + 4, 4, 12),  // Droite (+x)
                createOverlayMaterialForFace(texture, leftLegOverlayX + 0, leftLegOverlayY + 4, 4, 12),  // Gauche (-x)
                createOverlayMaterialForFace(texture, leftLegOverlayX + 4, leftLegOverlayY + 0, 4, 4),   // Haut (+y)
                createOverlayMaterialForFace(texture, leftLegOverlayX + 8, leftLegOverlayY + 0, 4, 4),   // Bas (-y)
                createOverlayMaterialForFace(texture, leftLegOverlayX + 4, leftLegOverlayY + 4, 4, 12),  // Devant (+z)
                createOverlayMaterialForFace(texture, leftLegOverlayX + 12, leftLegOverlayY + 4, 4, 12)  // Derrière (-z)
            ];

            // Créer l'overlay de la jambe gauche
            const leftLegOverlayGeometry = new THREE.BoxGeometry(4.5, 12.5, 4.5);
            const leftLegOverlay = new THREE.Mesh(leftLegOverlayGeometry, leftLegOverlayMaterials);
            ...
            const rightLegOverlayX = 0, rightLegOverlayY = 48;

            const rightLegOverlayMaterials = [
                createOverlayMaterialForFace(texture, rightLegOverlayX + 8, rightLegOverlayY + 4, 4, 12),  // Droite (+x)
                createOverlayMaterialForFace(texture, rightLegOverlayX + 0, rightLegOverlayY + 4, 4, 12),  // Gauche (-x)
                createOverlayMaterialForFace(texture, rightLegOverlayX + 4, rightLegOverlayY + 0, 4, 4),   // Haut (+y)
                createOverlayMaterialForFace(texture, rightLegOverlayX + 8, rightLegOverlayY + 0, 4, 4),   // Bas (-y)
                createOverlayMaterialForFace(texture, rightLegOverlayX + 4, rightLegOverlayY + 4, 4, 12),  // Devant (+z)
                createOverlayMaterialForFace(texture, rightLegOverlayX + 12, rightLegOverlayY + 4, 4, 12)  // Derrière (-z)
            ];

            // Créer l'overlay de la jambe droite
            const rightLegOverlayGeometry = new THREE.BoxGeometry(4.5, 12.5, 4.5);
            const rightLegOverlay = new THREE.Mesh(rightLegOverlayGeometry, rightLegOverlayMaterials);
        ...
        setStatus('Erreur lors de l\'application de la texture: ' + error.message, true);
        console.error('Erreur détaillée:', error);
...
function loadSkinByUsername(username) {
    if (!username) {
        console.error('Nom d\'utilisateur non valide:', username);
        username = '^^^^^^^^^^^^^^^^'; // Utiliser un nom par défaut
    }
    ...
    loadTexture(skinUrl, function (texture) {
        if (texture) {
            ...
            console.error('La texture est nulle après chargement');
            setStatus(`Erreur de chargement pour ${username}`, true);
        }
    });
}
...
window.addEventListener('load', function () {
    ...
    if (initScene()) {
        ...
        loadSkinByUsername(playerName);

        // Mettre à jour le titre de la page
        document.title = `Skin Minecraft - ${playerName}`;
