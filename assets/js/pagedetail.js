...
function updatePlayerUI(playerData) {
    ...
    document.getElementById('titre_pseudo_joueur').textContent = `Pseudo : ${playerData.name}`;
...
async function loadPlayerData() {
    ...
    try {
        const response = await fetch(`http://arthonetwork.fr:8001/apiP?player=${playerName}`);
        if (!response.ok) {
